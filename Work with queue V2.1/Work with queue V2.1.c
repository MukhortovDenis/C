/*
 ...
 Version     :
 Copyright   :
 Description : После окончания строки введите знак "|" для перехода на новую строку для чтения; поддерживается не более 50 символов.
               ПРИМЕР:
               Файл чтения: 6 3 / 7 1 - |
                            3 3 * |
               Файл вывода:
               ...
               Знак | - означает переход на новую строку, т.к структура строк поддерживает всего 50 знаков
...
typedef struct operation {//Очередь для строчек
    char command[50];
...
{//Специальня функция для очереди, которая позволяет читать нужные нам элементы
    if(head_q != NULL)
    {
        char *data_q;
...
//Специальная функция добавления элементво в очередь
...
int main()
{// Основная функция, работа с придаточными функциями
    FILE *input, *output;
    //Срез
    char *kick, *str;
